   ...
   <TransitionRoot appear :show="isVisible" as="template"
    > <TransitionChild
      ...
      <div :class="toastClasses" @click="hideToast">
         <Icon :name="getToastIcon" :class="iconClasses" /> <span>{{
          message
        }}</span
        > <button
          @click="hideToast"
        ...
        >
           <Icon name="heroicons:x-mark" class="h-4 w-4" /> </button
        >
      </div>
       </TransitionChild
    > </TransitionRoot
...
const { isVisible, message, type, hideToast, getToastIcon, getToastClasses } =
  useToast()
